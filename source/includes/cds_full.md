...
| nextInstalmentDate | loanDetails[0].nextPaymentDate | Loan Accounts API | mapped |
| minInstalmentAmount | loanDetails[0].repaymentAmount | Loan Accounts API | IF (replaymentType = PRINCIPAL_AND_INTEREST) THEN  = repaymentAmountIF (replaymentType = INT_ONLY) THEN  = 01. During P&I this is right value for both IO and P&I, but not during IO period. (Either calculate or make to change to CAP to calculate and provide) |
| minInstalmentCurrency |  |  | - |
| maxRedraw | - redrawEligible- advToDate | Loan Accounts API | IF (redrawEligible = true|Y) THEN  IF (advToDate >= minRedraw) THEN    = advToDate  ELSE    = 0ELSE  = 0 (account not eligible for Redraw)Extra amount re-paid until current date.1. Depends on minRedraw |
| maxRedrawCurrency |  |  | - |
| minRedraw |  |  | Hard code to "0.00" ?1. Get from Product Catalogue ?Needs to be fixed based on the sub-prod code for loan accounts, which we can get from the CAP (Not exposed on CAP, but has these values) |
| minRedrawCurrency |  |  | - |
| offsetAccountEnabled | productSetOffAccount (TBC) | Loan Accounts API | IF (X774662_SET_PSO_DDA_ACCT != 0) THEN  = true |
| offsetAccountIds | productSetOffAccount (TBC) | Loan Accounts API | mapped as array with one element1. Needs Id permanence? What if not authorised?IF (acct productCode is DDA in auth & acct is authorised/entitled) THEN  = apply ID PermELSE  not set |
